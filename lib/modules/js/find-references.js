...
function gatherRanges(binding) {
    ...
    ...binding.constantViolations
      ...
      .map(p => {
        ...
        if (['++', '--'].includes(node?.operator)) {
          range = locToRange(node.argument.loc)
          refRangesToRemove.push(range)
        } else {
  ...
  for (const rangeToRemove of refRangesToRemove) {
    const i = ranges.findIndex(
      range =>
        range.type === 'ref' &&
        range.start.row === rangeToRemove.start.row &&
        range.start.column === rangeToRemove.start.column &&
        range.end.row === rangeToRemove.end.row &&
        range.end.column === rangeToRemove.end.column
    )
    ranges.splice(i, 1)
  }
